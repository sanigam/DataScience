...
#write.csv(unique(tac$Account.Name), file = "account_name1.csv")

ac_cleanup <- read.csv("account_name.csv")
summary(tac[, -10])
tac <- merge(ac_cleanup, tac, by ="Account.Name", all.y = TRUE)
...
dt_cat <- rpart(RMA.Count~., data=tac[train,-c(1,3,4,8,10)], method = "class")

# Model to combine the 2 probabilities for RMA Y/N prediction
pred_text <- predict(dt_text, data=dfDTM[,-1], type = "prob")
pred_cat <- predict(dt_cat, tac[train,-c(1,3,4,8,10,9) ], type = "prob")
...
pred_cat <- predict(dt_cat, tac[test,-c(1,3,4,8,10,9) ], type = "prob")
...
#$RMA <- ifelse(predictions$Y>.5, "Y", "N")
...
pred <- function(iVec, iText) {
  ...
  dtm_test <- DocumentTermMatrix( cleanCorpus(Corpus(VectorSource(iText))),
              control=list(dictionary = Terms(dtm[["dtm"]])))   # Dictonary of terms from test set
  df_test <-  as.data.frame(data.matrix(dtm_test), stringsAsFactors = FALSE )
  pred_text <- predict(dt_text, df_test, type = "prob")
...
ui <- fluidPage(
  ...
  tags$p("This application predicts the probabilty of RMA in a Service Request. It also provides the trends for SR creation by various attributes."),
  ...
  inputPanel(
    selectInput(inputId = "iac", label = "Account",
                choices = ac , selectize = FALSE, selected="LINKEDIN"),
    selectInput(inputId = "itech", label = "Technology",
                choices = tech , selectize = FALSE, selected = "Hardware" ),
    selectInput(inputId = "isubtech", label = "Sub Technology",
                choices = subtech , selectize = FALSE, selected = "Redundant Power Supply (RPS)"),
    selectInput(inputId = "ihwp", label = "HW Platform",
                choices = hwp , selectize = FALSE, selected = "AIRCT5508")
  ),
  wellPanel(
